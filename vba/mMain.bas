...
Public Type CarData
    ...
    CarName    As String   ' 車名+グレード
    Score      As Variant  ' 評価点
    Price      As Variant  ' 落札価格
    Tax        As Variant  ' 消費税
    CarTax     As Variant  ' 自動車税
    Recycle    As Variant  ' リサイクル料
    AuctionFee As Variant  ' 落札手数料
    Venue      As String   ' オークション会場
...
Function GetNextWriteRow(ws As Worksheet) As Long
    ...
    Select Case qVal
        Case "着"
            GetNextWriteRow = lastRow + 1   ' 行2の次 = 新しい行1
        Case "予定"
            GetNextWriteRow = lastRow + 2   ' 行1の次の次 = 新しい行1
        Case Else
...
Sub WriteToSheet(ws As Worksheet, data As CarData)
    ...
    ws.Cells(r, 6).Value2  = data.CarName                ' F: 車名・グレード
    ws.Cells(r, 17).Value2 = "予定"                      ' Q: 固定
